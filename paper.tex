...
The per-slice generator (DSC\,74.9\%) applies $\mathcal{G}$ to each 2D slice independently using $\mathcal{L}_{\text{SPD}}$ only: $\mathcal{L}_{\text{ISC}}$ is excluded because a position-unaware 2D generator has no volumetric context to coordinate spectral patterns across slices---its gradients cannot drive systematic inter-slice diversity when adjacent inputs are perceptually similar. Consequently, the noise remains spectrally coherent across slices, leaving the volumetric continuity prior intact.
