...
Table~\ref{tab:main} reports the protection performance on both datasets. VoxShield achieved a dramatic
...
Table~\ref{tab:transfer} demonstrates that VoxShield perturbations transfer effectively across
...
Table~\ref{tab:ablation} validates the contribution of each component on FLARE21. Removing
...
inter-slice predictions, confirming that VoxShield uniquely targets the volumetric continuity prior on which 3D models depend.
...
VoxShield is the first unlearnable example framework for 3D medical image segmentation. Targeting inter-slice anatomical consistency as the key exploitable prior, we devise frequency-domain objectives disrupting it at both the noise-structure ($\mathcal{L}_{\text{ISC}}$) and semantic-prediction ($\mathcal{L}_{\text{SPD}}$) levels. State-of-the-art protection across two benchmarks and four architectures confirms that effective 3D data protection must attack the volumetric priors that 3D models fundamentally depend on.
