...
To corrupt not only low-level pixel statistics but also high-level semantic predictions, we maximize the discrepancy between surrogate outputs on clean and perturbed volumes.

Let $\bm{L}\!=\!\mathcal{F}_s(\bm{x};\theta)$ and
$\bm{L}'\!=\!\mathcal{F}_s(\bm{x}_p;\theta)$ denote the
